import { spDatoRequerimientoBF_Listar, spDatoRequerimiento_Consultar, spFormularioDiseno_Listar } from '../../database/qrys/index.js'
...
export const getBF = async (req, res) => {
    ...
    try {        
        ...
        data.registros = await spDatoRequerimientoBF_Listar(token, PageNumber, RowsOfPage)                         
...
export const getBFid = async (req, res) => {    
    ...
    try {
        ...
        if(error.message === 'Registro no encontrado'){
            res.status(404).json({"id":"bf","error":404,message:error.message});
            return;
        }
        res.status(500).json({"id":"bf","error":500,message:error.message});
    }
