import { spDatoRequerimientoBA_Listar, spDatoRequerimiento_Consultar, spFormularioDiseno_Listar } from '../../database/qrys/index.js'
...
export const getBA = async (req, res) => {
    ...
    try {        
        ...
        data.registros = await spDatoRequerimientoBA_Listar(token, PageNumber, RowsOfPage)                         
        res.status(200).json(data)        

    }catch (error) {
        if(error.message === 'No autorizado'){
            res.status(401).json({"id":"ba","error":401,message:error.message});
            return;
        }
        res.status(500).json({"id":"ba","error":500,message:error.message});
    }
}

export const getBAid = async (req, res) => {    
    const { id } = req.params
    //Rescatando datos del payload del token
    const token = req.cookies.access_token
    if(!token){
    ...
    try {
        ...
        if(error.message === 'Registro no encontrado'){
            res.status(404).json({"id":"ba","error":404,message:error.message});
            return;
        }
        res.status(500).json({"id":"ba","error":500,message:error.message});
    }
...
export const postBA = async (req, res) => {
    ...
    res.status(201).json({"message":"Guardando datos de bandeja de archivados"})
}

export const patchBAid = (req, res) => {
    const { id } = req.params
    ...
    res.status(201).json({"message":"Actualizando datos de bandeja de archivados con id: " + id})
}