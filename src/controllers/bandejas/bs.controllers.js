import { spDatoRequerimientoBS_Listar, spDatoRequerimiento_Consultar, spFormularioDiseno_Listar } from '../../database/qrys/index.js'
...
export const getBS = async (req, res) => {
    ...
    try {        
        ...
        data.registros = await spDatoRequerimientoBS_Listar(token, PageNumber, RowsOfPage)                         
        res.status(200).json(data)        

    }catch (error) {
        if(error.message === 'No autorizado'){
            res.status(401).json({"id":"bs","error":401,message:error.message});
            return;
        }
        res.status(500).json({"id":"bs","error":500,message:error.message});
    }
}
...
export const getBSid = async (req, res) => {    
    ...
    try {
        ...
        if(error.message === 'Registro no encontrado'){
            res.status(404).json({"id":"bs","error":404,message:error.message});
            return;
        }
        res.status(500).json({"id":"bs","error":500,message:error.message});
    }
...
export const patchBSid = (req, res) => {
    const { id } = req.params
